...
/**
 * REST controller for managing {@link id.tech.cakra.ecfcoresvc.domain.AccountBank}.
 */
@RestController
@RequestMapping("/api")
public class AccountBankResource {
    ...
    private static final String ENTITY_NAME = "ecfcoresvcAccountBank";
    ...
    public ResponseEntity<AccountBankDTO> updateAccountBank(@Valid @RequestBody AccountBankDTO accountBankDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, accountBankDTO.getId().toString()))
            .body(result);
    }
    ...
    public List<AccountBankDTO> getAllAccountBanks(@RequestParam(required = false) String filter) {
        if ("accountmember-is-null".equals(filter)) {
            log.debug("REST request to get all AccountBanks where accountMember is null");
            return accountBankService.findAllWhereAccountMemberIsNull();
        }
